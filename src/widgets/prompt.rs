...
impl<'a> Prompt<'a> {
  pub fn new(total_num_crates: u64, selected: u64, status: bool, mode: Mode, input: &'a tui_input::Input) -> Self {
    let vertical_margin = 1 + config::get().prompt_padding;
    let horizontal_margin = 1 + config::get().prompt_padding;
    Self { total_num_crates, status, selected, mode, input, vertical_margin, horizontal_margin }
  }
