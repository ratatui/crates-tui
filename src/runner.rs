...
pub async fn execute(tui: &mut Tui) -> Result<()> {
  ...
  loop {
    if let Some(e) = tui.next().await {
      match e {
        ...
        tui::Event::Key(key) => {
          ...
          if let Some(action) = app.handle_key_events(key, &last_tick_key_events)? {
            tx.send(action)?;
          }
    ...
    while let Ok(action) = rx.try_recv() {
      ...
      if let Some(action) = app.update(action.clone())? {
        tx.send(action)?
      };
      match action {
        Action::Tick => {
